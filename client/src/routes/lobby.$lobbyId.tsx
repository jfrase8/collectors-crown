...
function LobbyPage() {
  ...
  if (!lobby || lobby.id !== lobbyId || !playerId) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-4">
        <p className="text-secondary">You are not in this lobby.</p>
  ...
  if (lobby.phase === "in_progress") {
    return <GameScreen myPlayerId={playerId} />
  }
    ...
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-8 px-4 py-12">
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-xl font-semibold">
          Players ({lobby.players.length}/{MAX_LOBBY_PLAYERS})
        </h2>
      ...
      {isHost ? (
        <div className="flex flex-col items-center gap-2">
          <Button isDisabled={!canStart} onPress={start}>
            Start Game
          </Button>
          {!canStart && (
